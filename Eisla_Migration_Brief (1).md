...
- Set up Supabase project, create all tables from schema above (including `field_changes` and `access_log`)
- Create PostgreSQL trigger on `orders` and `customers` tables to auto-write to `field_changes` on UPDATE
...
- Data access logging middleware — log views/exports of orders and customer PII to `access_log`
...
- **Security audit log** — per-user: logins, logouts, lockouts, password changes, credential resets, account creation. Stored in `audit_log` table.
- **Order timeline** — every stage transition, sense check decision, pipeline run, quote sent, comms logged. Timestamped with actor name.
- **GDPR consent log** — per-customer: every consent toggle change with old value, new value, actor, timestamp.
- **Comms log** — emails, calls, meetings, notes logged per order with timestamp and user.

### Missing — Add During Migration
- **Field-level change audit** — track old/new values when any order or customer field is edited. Critical fields: tier, fee, mfg_estimate, payment_status, stage (manual overrides), service_level, description. Use a PostgreSQL trigger on the `orders` and `customers` tables that writes to a `field_changes` table on UPDATE.
- **Actor tracking on edits** — record which user changed which fields. Pass `current_user_id` via Supabase RLS context (`auth.uid()`).
- **Data access logging** — log when users view or export customer PII (relevant for GDPR Article 30 accountability). Implement as API middleware that logs to an `access_log` table.
...
field_changes
  id              UUID PRIMARY KEY
  table_name      TEXT NOT NULL        -- 'orders', 'customers'
...
access_log
  id              UUID PRIMARY KEY
  user_id         UUID REFERENCES users(id)
  action          TEXT NOT NULL        -- 'view_order', 'view_customer', 'export_data'
  resource_type   TEXT                 -- 'order', 'customer'
  resource_id     UUID
  ip              INET
  created_at      TIMESTAMPTZ DEFAULT now()
```

Implement `field_changes` as a PostgreSQL trigger so it captures changes regardless of which API route triggers the update. The `access_log` is application-level middleware.
